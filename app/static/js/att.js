    ...
    .then((result) => {
      ...
      next_left.addEventListener("click", () => {
        ...
        if (pic_order < 0) {
          ...
          img_page_display.childNodes[images.length].style.backgroundColor =
            "black";
        } else if (pic_order == 7) {
          ImageDisplayed.setAttribute("src", images[pic_order - 1]);
          img_page_display.childNodes[pic_order + 1].style.backgroundColor =
            "white";
          img_page_display.childNodes[pic_order].style.backgroundColor =
            "black";
          pic_order = pic_order - 1;
        } else {
          console.log(pic_order);
          ImageDisplayed.setAttribute("src", images[pic_order]);
          img_page_display.childNodes[pic_order + 1 + 1].style.backgroundColor =
            "white";
          img_page_display.childNodes[pic_order + 1].style.backgroundColor =
            "black";
        }
...
order_btn.addEventListener("click", () => {
  if (!loginExports.member_id) {
    loginExports.loginBoxController(
      order_btn,
      null,
      loginExports.signin_form_box,
      "block"
    );
  } else {
